...
void XWingRowHint::apply() {
    const Range &range1 = RANGES.get_column(col1);
    const Range &range2 = RANGES.get_column(col2);

    for (int row = 0; row < 9; ++row) {
        if (row != row1 && row != row2) {
            grid[range1[row]].remove_choice(value);
            grid[range2[row]].remove_choice(value);
        }
    }
}
...
void XWingColumnHint::apply() {
    ...
    for (int col = 0; col < 9; ++col) {
        if (col != col1 && col != col2) {
            grid[range1[col]].remove_choice(value);
            grid[range2[col]].remove_choice(value);
        }
    }
}
...
bool XWingHintProducer::check_second_col(int col, int row1, int row2,
    ...
    int idx2 = row2 * 9 + col;
    return grid[idx1].has_choice(value) && grid[idx2].has_choice(value)
            && count_col_value(grid, col, value) == 2 && (count_row_value(
            grid, row1, value) > 2 || count_row_value(grid, row2, value) > 2);
}
